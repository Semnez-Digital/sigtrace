...
pub fn read_p5(bytes: &[u8]) -> Result<Gray, String> {
    ...
    let h: usize = tok().ok_or("no height")?.parse().map_err(|_| "bad height")?;
    let maxv: usize = tok().ok_or("no maxval")?.parse().map_err(|_| "bad maxval")?;
    if maxv != 255 {
    ...
    Ok(Gray { w, h, px: bytes[pos..pos + need].to_vec() })
}
